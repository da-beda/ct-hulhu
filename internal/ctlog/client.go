...
func NewClientWithLogID(baseURL, logID string, timeout time.Duration, retries int) *Client {
	...
	if requestBase != "" && !strings.HasSuffix(requestBase, "/") {
		requestBase += "/"
	}
	return &Client{
		baseURL:   requestBase,
...
func (c *Client) Source() EntrySource {
	return EntrySource{Protocol: ProtocolRFC6962, LogID: c.logID, LogURL: c.sourceURL, Verified: false}
}
...
func (c *Client) GetTreeHead(ctx context.Context) (*TreeHead, error) {
	...
	root, err := base64.StdEncoding.DecodeString(sth.SHA256RootHash)
	if err != nil {
		return nil, fmt.Errorf("decoding STH root hash: %w", err)
	}
	if len(root) != 32 {
		return nil, fmt.Errorf("STH root hash has %d bytes, want 32", len(root))
	}
	sig, err := base64.StdEncoding.DecodeString(sth.TreeHeadSignature)
	if err != nil {
		return nil, fmt.Errorf("decoding STH signature: %w", err)
	}
...
func (c *Client) GetSTH(ctx context.Context) (*STH, error) {
	body, err := c.doRequestWithRetry(ctx, c.baseURL+"ct/v1/get-sth")
	if err != nil {
	...
	if sth.TreeSize < 0 {
		return nil, fmt.Errorf("STH contains negative tree size %d", sth.TreeSize)
	}
	return &sth, nil
}
...
func (c *Client) doRequest(ctx context.Context, url string) ([]byte, error) {
	...
	if len(body) > maxResponseSize {
		return nil, fmt.Errorf("response from %s exceeds %d bytes", url, maxResponseSize)
	}
	return body, nil
}
